...
def main():
    ...
    model = GPTMini(tok.vocab_size, block_size=cfg.get("block_size",256), num_styles=cfg.get("num_styles",1),
    ...
    out = model.generate(start, args.num_tokens, temperature=args.temperature, top_k=args.top_k, top_p=args.top_p, style=(torch.tensor([args.style], device=device) if args.style is not None else None))[0].tolist()
